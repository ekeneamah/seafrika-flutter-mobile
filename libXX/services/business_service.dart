...
class BusinessService {
  ...
  CollectionReference get businessCollection => _firestore.collection(CollectionNames.businesses);
  ...
  Future<String> createBusiness(Business business) async {
    try {
      ...
      if (!canCreate) {
        throw Exception('You do not have permission to create a business. Please ensure you have the correct role.');
      }
      ...
      final nameId = '${business.name.toLowerCase().replaceAll(' ', '_')}_${business.country.toLowerCase()}';
      ...
      final businessNameDoc = _firestore.collection(CollectionNames.businessNames).doc(nameId);
      ...
      final requiredFields = ['id', 'name', 'address', 'country', 'state', 'ownerId', 'isActive', 'createdAt'];
      ...
      if (e.toString().contains('permission-denied')) {
        throw Exception('You do not have permission to create a business. Please ensure you are logged in as a business owner.');
      }
  ...
  Future<bool> isBusinessNameAvailable(String businessName, String country) async {
    try {
      final nameId = '${businessName.toLowerCase().replaceAll(' ', '_')}_${country.toLowerCase()}';
      final doc = await _firestore.collection(CollectionNames.businessNames).doc(nameId).get();
  ...
  Future<bool> isUserBusinessMember(String businessId, String userId) async {
    try {
      ...
      return business.ownerId == userId || 
             business.adminIds.contains(userId);
    } catch (e) {
  ...
  Future<void> updateBusiness(String businessId, Map<String, dynamic> updates) async {
    try {
      ...
      if (updates.containsKey('name') || updates.containsKey('country')) {
        ...
        if (newName != currentBusiness.name || newCountry != currentBusiness.country) {
          final batch = _firestore.batch();
          
          // Delete old name entry
          final oldNameId = '${currentBusiness.name.toLowerCase().replaceAll(' ', '_')}_${currentBusiness.country.toLowerCase()}';
          final oldNameDoc = _firestore.collection(CollectionNames.businessNames).doc(oldNameId);
          batch.delete(oldNameDoc);
          
          // Create new name entry
          final newNameId = '${newName.toLowerCase().replaceAll(' ', '_')}_${newCountry.toLowerCase()}';
          final newNameDoc = _firestore.collection(CollectionNames.businessNames).doc(newNameId);
      ...
      // Check if it's a uniqueness violation
      if (e.toString().contains('already exists') || e.toString().contains('ALREADY_EXISTS')) {
        throw Exception('A business with this name already exists in the specified country. Please choose a different name.');
      }
  ...
  Future<void> deleteBusiness(String businessId) async {
    try {
      ...
      final nameId = '${currentBusiness.name.toLowerCase().replaceAll(' ', '_')}_${currentBusiness.country.toLowerCase()}';
      final nameDoc = _firestore.collection(CollectionNames.businessNames).doc(nameId);
  ...
  Future<bool> canUserCreateBusiness(String userId) async {
    try {
      ...
      final userDoc = await _firestore.collection(CollectionNames.users).doc(userId).get();
      ...
      if (!userData.containsKey('roles') || (userData['roles'] as List).isEmpty) {
        return true;
      }
  ...
  bool validateBusinessData(Business business) {
    ...
    if (business.phone != null && business.phone!.isNotEmpty) {
      ...
      if (!phoneRegex.hasMatch(business.phone!.replaceAll(RegExp(r'[\s\-\(\)]'), ''))) return false;
    }
