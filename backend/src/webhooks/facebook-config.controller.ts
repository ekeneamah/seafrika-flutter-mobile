...
export class FacebookConfigController {
  ...
  @Get('app-review-readiness')
  @ApiOperation({ summary: 'Check App Review readiness for Facebook/Instagram integration' })
  @ApiResponse({ status: 200, description: 'App Review readiness assessment' })
  async checkAppReviewReadiness() {
    const baseUrl = process.env.BASE_URL || 'https://seafrikaapi-u53tcgosiq-uc.a.run.app';
    const validation = await this.facebookConfigService.validateConfiguration();
    
    // Check all required components for App Review
    const checks = {
      ...
      privacy_policy_url: {
        status: false, // This needs to be manually configured in Facebook Dashboard
        requirement: 'Privacy Policy URL must be set in Facebook App Dashboard',
        current_value: 'Must be configured in Facebook Developer Console',
        is_required: true,
        manual_step: true
      },
      ...
      terms_of_service_url: {
        status: false, // This needs to be manually configured in Facebook Dashboard
        requirement: 'Terms of Service URL should be set in Facebook App Dashboard',
        current_value: 'Must be configured in Facebook Developer Console',
        is_required: false,
        manual_step: true
      },
      ...
      app_icon: {
        status: false, // This needs to be manually configured in Facebook Dashboard
        requirement: 'App icon must be uploaded (1024x1024px)',
        current_value: 'Must be uploaded in Facebook Developer Console',
        is_required: true,
        manual_step: true
      },
      ...
      app_description: {
        status: false, // This needs to be manually configured in Facebook Dashboard
        requirement: 'Clear app description explaining Instagram integration use case',
        current_value: 'Must be written in Facebook Developer Console',
        is_required: true,
        manual_step: true
      },
      ...
      permission_justification: {
        status: false, // This needs to be manually provided during review
        requirement: 'Clear justification for each requested permission',
        current_value: 'Must be provided during App Review submission',
        is_required: true,
        manual_step: true
      },
      
      demo_video: {
        status: false, // This needs to be manually created
        requirement: 'Demo video showing Instagram integration in action',
        current_value: 'Must be created and uploaded during App Review',
        is_required: true,
        manual_step: true
      },
      
      test_users: {
        status: false, // This needs to be manually configured
        requirement: 'Test users configured with access to Instagram features',
        current_value: 'Must be configured in Facebook Developer Console',
        is_required: true,
        manual_step: true
      }
    ...
    const requiredChecks = Object.values(checks).filter(check => check.is_required);
    const passedRequiredChecks = requiredChecks.filter(check => check.status).length;
    
    const overallScore = Math.round((passedChecks / totalChecks) * 100);
    const requiredScore = Math.round((passedRequiredChecks / requiredChecks.length) * 100);
    
    // Generate action items
    const actionItems = Object.entries(checks)
    ...
    return {
      ...
      required_items_score: requiredScore,
      summary: {
        total_checks: totalChecks,
        passed_checks: passedChecks,
        required_checks: requiredChecks.length,
        passed_required_checks: passedRequiredChecks,
        manual_steps_remaining: actionItems.filter(item => item.manual_step).length
      },
      technical_readiness: {
        status: passedRequiredChecks >= 6 ? 'READY' : 'NEEDS_WORK', // First 6 are technical
        endpoints_configured: true,
        https_enabled: checks.https_endpoint.status,
        oauth_flow_ready: checks.working_oauth_flow.status,
        privacy_endpoints_ready: checks.deauthorize_callback.status && checks.data_deletion_request.status
      },
      policy_readiness: {
        status: 'NEEDS_MANUAL_SETUP',
        privacy_policy_required: !checks.privacy_policy_url.status,
        app_details_required: !checks.app_description.status,
        app_icon_required: !checks.app_icon.status
      },
      review_readiness: {
        status: 'NEEDS_MANUAL_SETUP',
        demo_video_required: !checks.demo_video.status,
        test_users_required: !checks.test_users.status,
        permission_justification_required: !checks.permission_justification.status
      },
      ...
      next_steps: [
        {
          step: 1,
          title: 'Complete Manual Configuration in Facebook Developer Console',
          description: 'Set up privacy policy, app icon, description, and other required fields',
          items: actionItems.filter(item => item.manual_step && item.priority === 'HIGH')
        },
        {
          step: 2,
          title: 'Prepare App Review Materials',
          description: 'Create demo video and configure test users',
          items: [
            'Create a demo video showing Instagram login and data usage',
            'Add test users in Facebook Developer Console',
            'Prepare detailed use case descriptions for each permission'
          ]
        },
        {
          step: 3,
          title: 'Submit for App Review',
          description: 'Once all items are complete, submit your app for review',
          items: [
      ...
      helpful_links: {
        ...
        app_review_guide: 'https://developers.facebook.com/docs/app-review/',
        instagram_basic_display_review: 'https://developers.facebook.com/docs/instagram-basic-display-api/guides/getting-started',
        privacy_policy_generator: 'https://app-privacy-policy-generator.firebaseapp.com/',
        demo_video_guidelines: 'https://developers.facebook.com/docs/app-review/screencast'
      },
      current_endpoints: {
        oauth_redirect: `${baseUrl}/api/config/facebook/instagram/oauth/redirect`,
        deauthorize_callback: `${baseUrl}/api/config/facebook/instagram/deauthorize`,
        data_deletion_request: `${baseUrl}/api/config/facebook/instagram/data-deletion`,
        webhook_url: `${baseUrl}/api/webhooks/instagram`,
        readiness_check: `${baseUrl}/api/config/facebook/app-review-readiness`,
