  ...
  void _prefillProductDetails() {
    if (widget.prefilledProduct != null) {
      ...
      setState(() {
        ...
        _selectedProductImage = product.displayImageUrl ??
        ...
        _quantityController.text =
            product.stock.toString(); // Add current quantity
  ...
  Future<void> _saveInventory() async {
    ...
    try {
      ...
      if (businessContext == null) {
        _showSnackBar('No business selected. Please select a business first.',
            isError: true);
        return;
      }
      ...
      if (_isEditing && widget.inventory != null) {
        ...
        if (existingInventory != null) {
          ...
          _showSnackBar('Business inventory created successfully',
              isError: false);
        }
      }
      ...
      _showSnackBar('Failed to save inventory item: ${e.toString()}',
          isError: true);
    } finally {
      setState(() => _isLoading = false);
    }
      ...
      for (final productId in _selectedProductIds) {
        ...
        if (warehouseQty > 0) {
          ...
          await storeInventoryService.createStoreInventory(
            ...
            storeId: result['storeId'],
            businessInventoryId:
                inventory.id, // Updated to use businessInventoryId
            productId: product.id,
            productName: product.name,
            quantity: warehouseQty,
            minimumQuantity: 1,
